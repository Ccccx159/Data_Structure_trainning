...
Status push_stack_sq(p_sq_stack_t stk, elemType_sq_stack e) {
  if (stk->stack_len == stk->stackSize) {
    stk->base = (elemType_sq_stack *)realloc(
        stk->base,
        (stk->stackSize + 1 + STACK_INCREMENT) * sizeof(elemType_sq_stack));
    check_ptr(stk->base);
    stk->top = stk->base + stk->stack_len;
    stk->stackSize = stk->stackSize + STACK_INCREMENT;
  }
...
// 遍历
Status traverse_stack_sq(sq_stack_t stk, visit vfunc) {
  check_ptr(stk.base);
  if (0 == stk.stack_len) {
    std::cout << "stack is Empty!..." << std::endl;
    return ERROR;
